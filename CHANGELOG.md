...
- `POST /ai/suggest` had the same unescaped-filter issue in `ai.py`: `_fetch_ticket` and `_fetch_notes` interpolated the request body's `ticket_id` directly into PostgREST filters and were missed by the earlier sweep. They now route `ticket_id` through `database.escape_filter_value` as well.

...
- `POST /ai/suggest` no longer interpolates the request's `ticket_id` unescaped into PostgREST filters when fetching the target ticket and its notes, closing the same filter-injection gap already fixed for the `{id}`-based ticket routes.

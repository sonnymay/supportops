...
- `POST /ai/suggest` no longer interpolates the caller-supplied `ticket_id` directly into PostgREST filters when loading the ticket and its notes. This was the same filter-injection gap already closed for the REST routes in `main.py`, but `ai.py`'s helpers were missed; they now escape `ticket_id` via `escape_filter_value` as well.
